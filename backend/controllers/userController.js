...
const registerStudent = async (req, res) => {
    try {
        ...
        if (!name || !email || !department || !macAddress) {
            return res.status(400).json({ message: 'Missing required fields: name, email, department, macAddress' });
        }
        ...
        console.error('Error registering student:', error && error.stack ? error.stack : error);
